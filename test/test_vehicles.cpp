...
TEST(VehicleAuxTest, filter) {
    ...
    std::function<bool(const Vehicle&)> id_contains_KR = [](const Vehicle& vehicle) {

        bool containsChunk = (vehicle.get_id().find("KR") != std::string::npos);
        if (containsChunk) {
            return &vehicle;
        }
    };
    auto filtered_vehicles = filter_vehicles(vehicles.begin(), vehicles.end(), id_contains_KR);
